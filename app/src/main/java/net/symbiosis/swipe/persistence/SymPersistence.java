package net.symbiosis.swipe.persistence;
 ...
 * Time: 12:24 PM
 */
public class SymPersistence extends SQLiteOpenHelper {

	// If you change the database schema, you must increment the database version.
	private static final int DATABASE_VERSION = 2;

	private static final String TAG = getTag(SymPersistence.class);

	private static final String DATABASE_NAME = "SymMerchant.db";
    ...
    public SymPersistence(Context context) { super(context, DATABASE_NAME, null, DATABASE_VERSION); }
    ...
    public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS " + USER_DETAILS + " (" +
			"sym_user_id INTEGER PRIMARY KEY," +
			"wallet_id INTEGER," +
			"username VARCHAR(50)," +
	...
	private SQLiteDatabase getSymWritableDatabase() {
	...
	public String dumpDatabaseToString() {
		...
		for (String table : allTables) {
			Cursor dbCursor = getSymWritableDatabase().rawQuery("SELECT * FROM " + table, null);
	...
	public UserDetails getUserDetails() {
		Cursor userData = getSymWritableDatabase().rawQuery(
	...
	public void setUserDetails(UserDetails newUserDetails) {
        getSymWritableDatabase().execSQL("DELETE FROM " + USER_DETAILS);
		String sql = "INSERT INTO " + USER_DETAILS + "(sym_user_id,wallet_id,username,first_name,last_name,company_name,msisdn,email) VALUES (" +
        ...
        getSymWritableDatabase().execSQL(sql);
	}
}
